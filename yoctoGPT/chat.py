...
rolling context window. In raw mode (--raw), user input is fed as plain text
continuation, which is the correct interface for a base language model. In
formatted mode (default), turns are wrapped as User:/Assistant: for
...
from .advanced_model import AdvancedGPT, AdvancedModelConfig
from .performance_model import PerformanceGPT, PerformanceModelConfig
...
def parse_args():
    ...
    p.add_argument("--raw", action="store_true",
                   help="Raw continuation mode for base LMs (no chat formatting)")
    p.add_argument("--prime", type=str, default=None,
                   help="Initial text to seed the model context (raw mode)")
...
def main() -> None:
    ...
    if args.raw:
        _run_raw(args, encode, decode, eos_token, model, device)
    else:
        _run_formatted(args, encode, decode, eos_token, model, device)
